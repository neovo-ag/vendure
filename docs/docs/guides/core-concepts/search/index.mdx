...
The [`@vendure-community/elasticsearch-plugin`](https://github.com/vendurehq/community-plugins) stores the search index in an Elasticsearch cluster. It is designed for production workloads with large catalogs where you need more advanced full-text search capabilities, better performance at scale, or features like fuzzy matching and relevance tuning.
...
### Pagination with DefaultSearchPlugin

`DefaultSearchPlugin` applies [`shopListQueryLimit` and `adminListQueryLimit`](/reference/typescript-api/configuration/api-options/) to the `take` input in the corresponding API. A value above the configured limit is rejected.

When omitted, `take` defaults to 25, or the configured limit if it is lower, and `skip` defaults to 0. Both values must be non-negative integers. Setting `take: 0` returns an empty `items` array, while still allowing you to query `totalItems` and aggregations.

### Facet visibility with DefaultSearchPlugin

Shop search excludes private facets and their values from facet aggregations and from each item's `facetIds` and `facetValueIds`. Returned values must belong to the active channel. These checks use current facet data, so making a facet private hides its metadata even before the search index is refreshed. Admin search retains private facet visibility.

Hiding facet metadata does not remove otherwise matching products or change `totalItems`. Filtering by a known private or out-of-channel facet value ID still uses indexed associations and can affect matches and counts. Matching products and counts can therefore reveal associations even though the facet data itself is hidden.
