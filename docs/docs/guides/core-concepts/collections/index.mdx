---
title: 'Collections'
metaTitle: "Organizing Products with Collections in Vendure"
metaDescription: "Collections group product variants for navigation, promotions, and storefront display. Covers built-in filters and filter inheritance."
---
...
The `Product.collections` field returns only collections assigned to the active [channel](/core-concepts/channels/). The Shop API also excludes private collections; the Admin API can return private collections within the active channel. A collection appears once even when it contains several variants of the product.
...
Collections often work hand-in-hand with [facets and filters](/core-concepts/facets-filters/) to provide structured navigation in your storefront.
...
For details on creating custom collection filters, see the [Developer Guide](/developer-guide/strategies-configurable-operations/).
